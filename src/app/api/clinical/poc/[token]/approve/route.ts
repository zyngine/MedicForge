...
export async function POST(
  ...
  try {
    ...
    await adminAny
      .from("clinical_poc_tokens")
      .update({ action_taken: "approved", action_taken_at: now })
      .eq("id", tokenRow.id);
    ...
    if (student?.email) {
      ...
      const template = clinicalShiftApprovedEmail({
        ...
        address: site ? `${site.address || ""}, ${site.city || ""}, ${site.state || ""}`.trim() : "",
        scheduleUrl: `${appUrl}/student/clinical/my-shifts`,
      });
    ...
    console.error("[POC Approve] Error:", err);
