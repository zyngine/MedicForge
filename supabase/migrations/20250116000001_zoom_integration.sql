...
CREATE TABLE video_session_attendance (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    tenant_id UUID NOT NULL REFERENCES tenants(id) ON DELETE CASCADE,
...
-- Indexes
CREATE INDEX idx_zoom_connections_user ON zoom_connections(user_id);
CREATE INDEX idx_zoom_connections_tenant ON zoom_connections(tenant_id);
CREATE INDEX idx_video_sessions_course ON video_sessions(course_id);
CREATE INDEX idx_video_sessions_creator ON video_sessions(created_by);
...
CREATE INDEX idx_video_attendance_session ON video_session_attendance(session_id);
CREATE INDEX idx_video_attendance_user ON video_session_attendance(user_id);
...
-- Zoom connections: Users can only see their own
